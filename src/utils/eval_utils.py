...
def initiate_model(args, ckpt_path):
    ...
    if args.model_type == 'ViLa_MIL':
        ...
        config.input_size = 512 if args.use_plip_only else 1024
        config.hidden_size = 192
        config.text_prompt = args.text_prompt
        config.prototype_number = args.prototype_number
        config.use_uot = args.use_uot
        config.tau_a = args.tau_a
        config.tau_b = args.tau_b
        config.ratio_graph = args.ratio_graph
        config.freeze_textEn = args.freeze_textEn
        config.typeGNN = args.typeGNN
        config.use_plip = args.use_plip
        config.dual_mlp = args.dual_mlp
        config.mlp_ratio = args.mlp_ratio
        
        config.use_plip_only = args.use_plip_only
        config.random_init_MLP = args.random_init_MLP
        config.use_standard_attention = args.use_standard_attention
...
def summary(mode, model, loader, args):
    ...
    if(mode == 'transformer'):
        for batch_idx, (data_s, coord_s, nodes_s, edges_s, data_l, coord_l, nodes_l, edges_l, label) in enumerate(loader):
            ...
            nodes_s, edges_s, nodes_l, edges_l = nodes_s.to(device), edges_s.to(device), nodes_l.to(device), edges_l.to(device)
            slide_id = slide_ids.iloc[batch_idx]
            with torch.no_grad():
                Y_prob, Y_hat, loss = model(data_s, coord_s, nodes_s, edges_s, data_l, coord_l, nodes_l, edges_l, label)

...
class Generic_WSI_Classification_Dataset(Dataset):
    def __init__(self,
        ...
        slide_data = pd.read_csv(csv_path).dropna()
        slide_data.columns = ['id', 'filename', 'md5', 'case_id', 'label', 'slide_id']
    ...
    def get_split_from_df(self, all_splits, split_key='train'):
        ...
        if len(split) > 0:
            ...
            split = Generic_Split(
                df_slice, data_dir_s=self.data_dir_s,
                data_graph_dir_s=self.data_graph_dir_s,
                data_dir_l=self.data_dir_l, data_graph_dir_l=self.data_graph_dir_l,
        ...
        if from_id:
            if len(self.train_ids) > 0:
                ...
                train_split = Generic_Split(
                    train_data, data_dir_s=self.data_dir_s,
                    data_dir_l=self.data_dir_l, mode=self.mode,
                    num_classes=self.num_classes
                )
            else:
            ...
            if len(self.val_ids) > 0:
                ...
                val_split = Generic_Split(
                    val_data, data_dir_s=self.data_dir_s,
                    data_dir_l=self.data_dir_l, mode=self.mode,
                    num_classes=self.num_classes
                )
            else:
            ...
            if len(self.test_ids) > 0:
                ...
                test_split = Generic_Split(
                    test_data, data_dir_s=self.data_dir_s,
                    data_dir_l=self.data_dir_l, num_classes=self.num_classes
                )
            else:
                test_split = None
        else:
    ...
    def __getitem__(self, idx):
        ...
        if not self.use_h5:
            if self.data_dir_s and self.data_dir_l:
                ...
                graph_scale_s = load_pickle_file(os.path.join(self.data_graph_dir_s, f"{slide_id}.pickle"))
                graph_scale_l = load_pickle_file(os.path.join(self.data_graph_dir_l, f"{slide_id}.pickle"))
                ...
                return features_s, coords_s, nodes_s, edges_s, features_l, coords_l, nodes_l, edges_l, label
        else:
